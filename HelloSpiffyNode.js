...
/*
 * Our HelloSpiffyNode object defines our server and routines it needs
...
var HelloSpiffyNode = {
    ...
    runServer: function() {
            ...
            path.exists(filename, function(exists) {  
                ...
                if (!exists) {  
                    if (uri.indexOf('/simple/') === 0) {            
                         ...
                         */
                        var resource  = HelloSpiffyNode.getI18nDateResource(request, uri);
        ...
        fs.readdir(path.join(process.cwd(), targetDir,  i18nDir), function(err, files) {
            HelloSpiffyNode.resources = files;
    ...
    getI18nDateResource: function(request, uri) {
        ...
        for (i = 0; i < locales.length; i++) {
            ...
            if (HelloSpiffyNode.resourceExists(resource)) {
                return resource;
            } else {
                /*
                 * Try just on country
    ...
    resourceExists: function(resource) {
        var i = 0;
        for (i = 0; i < HelloSpiffyNode.resources.length; i++) {
            if (HelloSpiffyNode.resources[i] === resource) {
                return true;
            }
        }
        return false;
